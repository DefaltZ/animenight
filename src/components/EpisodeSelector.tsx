  ...
  return (
    <div>
      <div className="flex flex-wrap text-white bg-gray-800 justify-center">
        <p className="text-lg text-red-500 font-bold p-5">Currently watching episode {props.currentEpisode}</p>
        <input
          type="text"
          value={searchInput}
      ...
      <div className="flex flex-wrap justify-center p-5" style={{ height: '200px', overflowY: 'auto' }}>
        {filteredEpisodes.map((episode: IAnimeEpisode) => (
          <div
            key={episode.number}
            className={`text-white h-16 w-16 flex items-center justify-center hover:bg-red-400 ${
              episode.number === props.currentEpisode ? 'bg-red-500' : 'bg-gray-900'
